...
for block_idx = 1:num_blocks
    ...
    for i = 1:length(selected_indexes)
        ...
        [high_values, low_values, high_indices, low_indices, selected_indices] = find_plateaus_time(filtered_data, threshold, median_threshold, max_plateaus, 'plateaus.mat', data_cell{selected_indexes(i)}, T.TimeInSeconds, color_order{block_idx}(i,:));
        ...
        selected_timestamps = T.Timestamp(selected_indices);
        ...
        fprintf('Median = %.3f, IQR =  %.3f\n', low_median, low_IQR);
        fprintf('Selected timestamps:\n');
        for i = 1:length(selected_timestamps)
            fprintf('Point %d: %s\n', i, string(selected_timestamps(i)));
        end
    end
