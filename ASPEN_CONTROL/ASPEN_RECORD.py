...
measurements_dir = join(Example_dir,"_myTMSi", '../ASPEN_EMG_MEASUREMENTS') # directory with all measurements
...
try:
    ...
    if (len(discoveryList) > 0):
        ...
        # Set the sample rate of the BIP and AUX channels to 4000/channel_divider Hz, where channel_divider = 2,...,8 (e.g. 8 -> 500 Hz)
        ...
        AUX_list = [] #[0,1,2] 
        BIP_list = [0,1] # Referring to two muscles signals
        ...
        # Initialise a file-writer class (XDF-format / POLY5-format) and state its file path
        #file_writer = FileWriter(FileFormat.xdf, join(measurements_dir,"Measurement.xdf"))
        file_writer = FileWriter(FileFormat.poly5,join(measurements_dir,"Measurement.poly5"))
